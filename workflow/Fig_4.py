...
merged_data_plot = merged_data.plot(ax=ax, alpha=1, cmap='Greens', column='frequency_per_year_nat', markersize=100, edgecolor='grey',zorder=5, vmin=vmin, vmax=vmax)
cbar = plt.colorbar(scalar_mappable, ax=ax, orientation='horizontal', pad=-0.2, shrink=0.5, extend='max',ticks=ticks) 
...
cbar.ax.set_xlabel('Events/year', fontsize=12,labelpad=15)  # Adjust label size
...
plt.savefig(path + 'ave_frequency_nat.png',dpi=600,bbox_inches='tight')
...
merged_data_plot = merged_data.plot(ax=ax, alpha=1, cmap='Greens', column='frequency_per_year_gaged', markersize=100, edgecolor='grey',zorder=5, vmin=vmin, vmax=vmax)
...
ax.text(0.8, 0.505, 'Neches R.', transform=ax.transAxes, fontsize=8,rotation = -40, color='dodgerblue')
ax.text(0.75, 0.60, 'Sabine R.', transform=ax.transAxes, fontsize=8,rotation = -24, color='dodgerblue')
plt.savefig(path + 'ave_frequency_gaged.png',dpi=600,bbox_inches='tight')
...
vmin = -100
vmax = 100
merged_data['relative_change_frequency'] *= 100
merged_data_plot = merged_data.plot(ax=ax, alpha=1, column='relative_change_frequency', markersize=100, edgecolor='grey', zorder=5, vmin=vmin, vmax=vmax, cmap='bwr')
...
ticks = np.arange(vmin, vmax + tick_interval, tick_interval)
# Add a colorbar with smaller size inside the figure frame
cbar = plt.colorbar(scalar_mappable, ax=ax, orientation='horizontal', label='Relative change (%)', pad=-0.2, shrink=0.5, extend='both', ticks=ticks) 
cbar.ax.xaxis.set_ticks_position('top')  # Move ticks to the top
cbar.ax.xaxis.set_label_position('top')  # Move label to the top
cbar.ax.tick_params(labelsize=10)  # Adjust tick label size
...
ax.text(0.8, 0.505, 'Neches R.', transform=ax.transAxes, fontsize=8,rotation = -40, color='dodgerblue')
ax.text(0.75, 0.60, 'Sabine R.', transform=ax.transAxes, fontsize=8,rotation = -24, color='dodgerblue')
plt.savefig(path + 'ave_frequency_change.png',dpi=600,bbox_inches='tight')
...
data[[ 'frequency_per_year_nat','frequency_per_year_gaged']].boxplot(ax=ax1, positions=positions, patch_artist=False, boxprops=boxprops, medianprops=medianprops, whiskerprops=whiskerprops, flierprops=flierprops, widths=0.6, showfliers=True)  
plt.grid(False)
...
ax2.boxplot(data['relative_change_frequency']*100, positions=[3], patch_artist=False, boxprops=boxprops, medianprops=medianprops, whiskerprops=whiskerprops, widths=0.6,flierprops=flierprops)  # Adjust the widths as needed

ax1.set_xticks([1, 2, 3])
ax1.set_xticklabels(['Natural Q','Regulated Q', 'Relative change'], rotation=0, fontsize = 14)
ax1.set_ylabel('Events per year', fontsize = 15,labelpad=10)
...
plt.grid(False)
ax1.text(0.08, 0.10, '(d)', transform=ax1.transAxes, fontsize=17) # , fontweight='bold', va='top'
plt.tight_layout()
plt.savefig(path + 'ave_frequency_box.png',dpi=600,bbox_inches='tight')