...
scalar_mappable = plt.cm.ScalarMappable(cmap='Greens')
...
merged_data_plot = merged_data.plot(ax=ax, alpha=1, cmap='Greens', column='max_timescale_nat', markersize=100, edgecolor='grey',zorder=5, vmin=vmin, vmax=vmax)

cbar = plt.colorbar(scalar_mappable, ax=ax, orientation='horizontal', pad=-0.2, shrink=1,ticks=ticks) 
...
cbar.ax.set_xlabel('Response time (months)', fontsize=12,labelpad=15)  # Adjust label size

# Specify the location of the colorbar
cbar.ax.set_position([0.1, 0.15, 0.5, 0.02])  # [left, bottom, width, height]

ax.text(0.02, 0.92, '(a) natural Q', transform=ax.transAxes, fontsize=17)
...
plt.savefig(path + 'response_nat.png',dpi=600,bbox_inches='tight')
...
merged_data_plot = merged_data.plot(ax=ax, alpha=1, cmap='Greens', column='max_timescale_gaged', markersize=100, edgecolor='grey',zorder=5, vmin=vmin, vmax=vmax)
...
ax.text(0.02, 0.92, '(b) observed Q', transform=ax.transAxes, fontsize=17)
ax.tick_params(axis='both', which='both', length=0)
ax.set_xticklabels([])
ax.set_yticklabels([])
...
plt.savefig(path + 'response_gaged.png',dpi=600,bbox_inches='tight')
...
vmin = np.min(100*np.abs(merged_data['max_timescale_relative_change']))
vmax = np.max(100*np.abs(merged_data['max_timescale_relative_change']))
merged_data['relative_change_ave_duration'] *= 100
merged_data_plot = merged_data.plot(ax=ax, alpha=1, column=(100*np.abs(merged_data['max_timescale_relative_change'])), markersize=100, edgecolor='grey', zorder=5, vmin=vmin, vmax=vmax, cmap='Oranges')

scalar_mappable = plt.cm.ScalarMappable(cmap='Oranges') 
scalar_mappable.set_array([])  
...
ticks = np.arange(vmin, vmax + tick_interval, tick_interval)
# Add a colorbar with smaller size inside the figure frame
cbar = plt.colorbar(scalar_mappable, ax=ax, orientation='horizontal', label='Relative change (%)', pad=-0.2, shrink=0.5, extend='max', ticks=ticks) #, pad=-0.2,shrink = 0.5 , ticks=np.linspace(-50, 50,21)
cbar.ax.xaxis.set_ticks_position('top')  # Move ticks to the top
cbar.ax.xaxis.set_label_position('top')  # Move label to the top
cbar.ax.tick_params(labelsize=10)  # Adjust tick label size
...
plt.savefig(path + 'response_time_change.png',dpi=600,bbox_inches='tight')
...
data[[ 'max_timescale_nat','max_timescale_gaged']].boxplot(ax=ax1, positions=positions, patch_artist=False, boxprops=boxprops, medianprops=medianprops, whiskerprops=whiskerprops, flierprops=flierprops, widths=0.6, showfliers=True)  # Adjust the widths as neededax1.set_ylabel('Response time (month)')
plt.grid(False)
...
ax2.boxplot(data['max_timescale_relative_change']*100, positions=[3], patch_artist=False, boxprops=boxprops, medianprops=medianprops, whiskerprops=whiskerprops, widths=0.6,flierprops=flierprops)  # Adjust the widths as needed

ax1.set_xticks([1, 2, 3])
ax1.set_xticklabels([ 'Natural Q', 'Regulated Q','Relative change'], rotation=0, fontsize = 14)
ax1.set_ylabel('Response time (months)', fontsize = 15,labelpad=10)
...
plt.savefig(path + 'response_time_box.png',dpi=600,bbox_inches='tight')